...
# -------- LABARATORY --------
def insert_laboratory(n=100000):
...
def insert_medicine(n=500000):
    medicines_with_purpose = {
        ...
        'Atorvastatin': 'Reduces bad cholesterol',
        ...
        'Albuterol': 'Treats breathing problems (asthma)',
    ...
    with engine.begin() as conn:
        for _ in tqdm(range(n), desc="Inserting medicines"):
            medicine_name = random.choice(list(medicines_with_purpose.keys()))
            dosage = f'{random.randint(1, 250)} mg'
            purpose = medicines_with_purpose[medicine_name]
            manufacturer = random.choice(manufacturers)
           

            query = text("""
                INSERT INTO medicine (medicine_name, dosage, purpose, manufacturer)
                VALUES (:medicine_name, :dosage, :purpose, :manufacturer)
            """)
            conn.execute(query, {
